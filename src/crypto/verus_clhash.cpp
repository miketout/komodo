    ...
    for (int64_t i = 0; i < 64; i++)
    {
        ...
        *(pMoveScratch++) = prandex;        
        ...
        switch (selector & 0x1c)
        {
            ...
            case 0x18:
            {
                const __m128i temp1 = _mm_load_si128(prandex);  //was randex
                const __m128i temp2 = _mm_load_si128(pbuf);     //was (pbuf - (((selector & 1) << 1) - 1));
                const __m128i add1 = _mm_xor_si128(temp1, temp2);

                // cannot be zero here
                const int32_t divisor = (uint32_t)(selector >> 32);   //was (uint32_t)selector;

                acc = _mm_xor_si128(add1, acc);  //same

                const int64_t dividend = _mm_cvtsi128_si64(acc);   //same
                const __m128i modulo = _mm_cvtsi32_si128(dividend % divisor);  //same
                acc = _mm_xor_si128(modulo, acc);  //same 

                const __m128i tempa1 = _mm_clmulepi64_si128(acc, temp1, 0x10);   //was _mm_mulhrs_epi16
                const __m128i tempa2 = _mm_xor_si128(tempa1, temp1);  //same

                if (dividend & 1)
                {
                    const __m128i temp12 = _mm_load_si128(prand);  //wa prandex
                    _mm_store_si128(prand, tempa2);  //was prandex

                    const __m128i temp22 = _mm_load_si128(pbuf); //same
                    const __m128i add12 = _mm_xor_si128(temp12, temp22);   //same
                    const __m128i clprod12 = _mm_mulhrs_epi16(add12, add12);     //was _mm_mulhrs_epi16
                    acc = _mm_xor_si128(clprod12, acc); //same
                    const __m128i clprod22 = _mm_clmulepi64_si128(temp22, temp22, 0x10); //same
                    acc = _mm_xor_si128(clprod22, acc); //same

                    const __m128i tempb1 = _mm_mulhrs_epi16(acc, temp12); //same
                    const __m128i tempb2 = _mm_xor_si128(tempb1, temp12);//same
                    _mm_store_si128(prandex, tempb2);//same
                }
                else
                {
                    const __m128i tempb3 = _mm_load_si128(prandex);  //was prand
                    _mm_store_si128(prandex, tempa2);//same
                    _mm_store_si128(prand, tempb3);//same
                }
                break;
                /*  ***old case 0x18
